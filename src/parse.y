...
%destructor select {
                    #ifdef SELECT_FUNC
                     sqlite3SelectDelete(pParse->db, $$);
                    #endif
                   }
%type selectnowith {Select*}
%destructor selectnowith {
                          #ifdef SELECT_FUNC
                           sqlite3SelectDelete(pParse->db, $$);
                          #endif
                         }
...
%ifndef SQLITE_OMIT_SUBQUERY
  ...
  expr(A) ::= expr(A) in_op(N) nm(Y) dbnm(Z) paren_exprlist(E). [IN] {
    ...
    Select *pSelect;
    #ifdef SELECT_FUNC
    pSelect = sqlite3SelectNew(pParse, 0,pSrc,0,0,0,0,0,0,0);
    #endif
...
cmd ::= PRAGMA nm(X) dbnm(Z).                {
                                              #ifdef PRAGMA_FUNC
                                               sqlite3Pragma(pParse,&X,&Z,0,0);
                                              #endif
                                             }
cmd ::= PRAGMA nm(X) dbnm(Z) EQ nmnum(Y).    {
                                              #ifdef PRAGMA_FUNC
                                              sqlite3Pragma(pParse,&X,&Z,&Y,0);
                                              #endif
                                             }
...
%destructor trigger_cmd_list {
#ifdef TRIGGER_FUNC
sqlite3DeleteTriggerStep(pParse->db, $$);
#endif}
